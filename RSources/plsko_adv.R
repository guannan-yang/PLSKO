...
library(mixOmics)
set.seed(7749)
plsko.adv <- function(X, threshold.abs = NULL, threshold.q = NULL, ncomp = 2, sparsity = 1, gauss.err = F, linear.thre = nrow(X)/2){
  n <- nrow(X)
  p <- ncol(X)
  ...
  if(!is.null(threshold.q)){
    threshold <- quantile(unlist(abs(cor_mat)), prob = threshold.q, names = F, na.rm = T)
  } 
  else if(!is.null(threshold.abs)){
    threshold <- threshold.abs
  }
  else {
    threshold <- quantile(unlist(abs(cor_mat)), prob = 0.95, names = F, na.rm = T)
  }
 ...
 for (i in 1:p){
    ...
    print(i)
      ...
      X.nb <- X[,nb]
      X_k.nb <- X_k[, k.nb]
      # X.rank <- qr(X.nb)$rank # wont work because when noise present r.numerical > real.r; still non-invertible
      ...
      if(length(nb) ==0){
        ...
        Y.hat <- linear.regression.generator(Y, X.run)
          ...
          if(is.null(ncomp)){
            this.ncomp <- ceiling(min(n/2, ncol(X.nb)/2, ncol(X_k.nb)/2))
          }
        ...
        #   }
        #   else this.ncomp <- ncomp
        #   this.ncomp <- min(this.ncomp, ncol(X_k.nb)-1)
        #   
        #   keepX <- list(X.nb = rep(round(sparsity*ncol(X.nb)), this.ncomp),
        #                 X_k.nb = rep(round(sparsity*ncol(X_k.nb)), this.ncomp))
        #   
        #   #design matrix
        ...
        #   
        #   
        #   Y.hat <- block.pls.recovery.generator(Y, X.run, ncomp = this.ncomp, design = des.mat, keepX = keepX)
...
#   n = nrow(X[[1]])
#   pls <- block.spls(X, Y, ncomp = ncomp, scale = F, mode = "regression", design = design)
...
#   n <- nrow(X)
#   p <- ncol(X)
#   pls <- spls(X, Y, ncomp = ncomp, scale = F, mode = "regression")
...
fit.type <- function(X, threshold.abs = NULL, threshold.q = NULL){
  n <- nrow(X)
  p <- ncol(X)
  mu <- colMeans(X)  
  ...
  for (i in 1:p){
      ...
      if(length(nb) ==0){
        ...
        var.type[i] <- 'block.pls'
        }
      }
  }
  return(list(thres.cor = threshold,
              fit.type = cbind(type = var.type, var.names = var.names),
