...
#' @export
stat.glmnet_coefdiff_edited <- function(X, X_k, y, family='gaussian', cores=2, ...) {
  ...
  W = W * (1-2*swap)
  Z[orig] = Z[orig]*(1-2*swap)
  Z[orig+p] = Z[orig+p]*(1-2*swap)  
...
ko.filter_lasso.test <- function(X, Xk, y, q, method = "lasso.lcd", sparsity = 0.1, data.log = T){
  n = nrow(X)
  p = ncol(X)
  
  W_Z <- stat.glmnet_coefdiff_edited(X, Xk, y)
  W <- W_Z$W
  ...
  return(list(S = S, S.plus = S.plus, W_Z = W_Z))
}