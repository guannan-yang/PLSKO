...
plsko <- function(X, nb.list = NULL, threshold.abs = NULL, threshold.q = NULL, ncomp = NULL, sparsity = 1, rmax = 5, seed = Sys.time(), simpls = F){
  set.seed(seed)
...
# Implement PLSKO with simpls algorithm
#' Calculate \eqn{\hat{X}} by fitting PLS regression on its neighbours using simpls algorithm
#'
#' @rdname simpls.recovery.generator
#' @keywords internal
#'
simpls.recovery.generator <- function(Y, X, ncomp){
  X <- as.data.frame(X)
  #X <- X[!duplicated(as.list(X))]
  n <- nrow(X)
  p <- ncol(X)
  simpls <- plsr(Y ~ ., data = X, ncomp = ncomp, method = "simpls", validation = "none")
  Y.hat <- simpls$fitted.values[1:n, 1, ncomp]
  return(Y.hat)
}
