...
plsAKO <- function(X, y, n_ko = 25,
  ...
  if(parallel){
    ...
    foreach::foreach(i = 1:n_ko, .combine = 'c') %dopar% {
      set.seed(seed + i)
...
#' @param Xko.list A list of knockoff variable matrices (\eqn{n \times p}) corresponding to the original matrix `X`.
#' @param q A numeric value of the target false discovery rate (FDR) level. Default is `0.05`.
#' @param w.method A string specifying the method to compute test statistics. Options are `"lasso.lcd"`, `"lasso.logistic"`, `"lasso.max.lambda"`, `"RF"`. Default is `"lasso.lcd"`.
#' @param offset A numeric value to specify the adjustment in empirical p-value calculation. Options are `0` or `1`. Default is `1`.
#' @param gamma A numeric value for quantile aggregation in the multiple knockoff p-value aggregation. Default is `0.3`.
#' @param seed A numeric value for the random seed. Default is `1`.
#'
#' @return A list of class 'AKO.result', containing:
#' \describe{
#'   \item{`s`}{A list of results from individual knockoff filters.}
