#' Importance statistics based on a GLM with cross-validation with Covariates (Adapted from `knockoff` package)
#' 
...
#' @param covariates Optional. A matrix or data frame of covariates to be included in the model, e.g. confounders. Default is NULL.
...
#' @export
stat.glmnet_coefdiff <- function(X, X_k, y, covariates = NULL, family='gaussian', cores=2, ...) {
  ...
  if(is.null(covariates)){
    glmnet.coefs = cv_coeffs_glmnet(cbind(X.swap, Xk.swap, covariates), y, family=family, parallel=parallel, ...)
  }
  else{
    n_cov <- ncol(covariates)
    penalty.factor <- c(rep(1, ncol(X.swap) + ncol(Xk.swap)), rep(0, n_cov)) # penalize only X and Xk
    glmnet.coefs = cv_coeffs_glmnet(cbind(X.swap, Xk.swap, covariates), y, family=family, parallel=parallel, 
