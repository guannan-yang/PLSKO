...
The result shows that the PLSKO method selected the 9 important variables with a false discovery proportion (FDP) of 0.1, which is a little higher than the target FDR level of 0.05. This might due to the small sample size and the high-dimensional setting, modified FDR control by `offset = 0` is used in this run, lack of specified neighbourhood information, or purely randomness in knockoff variable generating (given knockoff frame work only ensure FDR control as an expectation of FDP, which might fluctuate in different runs). 



#### Customized settings 1: Customised neighbourhood information
...
fdp(result)
```
These two options are equivalent, and you can choose the one that is more convenient for you. The result shows that the PLSKO method selected 3 important variables with a FDP of 0, however the power is low in this run.

#### Customized settings 2: Customised number of components and sparsity level
...
fdp(result)
```
We observed that the PLSKO method selected 7 important variables (out of 25) with a FDP of 0, with increased power compared to the previous run.

#### Customized settings 3: Binary response
...
fdp(result)
```
The result shows that the PLS-AKO method selected 7 important variables with a FDP of 0, 
